...
df_hr <- data.frame(
  time = c(cuts * 12, 24), # Map exactly to Months natively!
...
p_late <- ggplot(df_res %>% filter(Epoch == "Late post-LTFU"), aes(x = HR, y = Trial)) +
  ...
  labs(title = "Panel C: Late post-LTFU (months 6-24)",
       x = "Hazard Ratio (log scale)", y = "")

layout <- pA / (p_early | p_late)
out_name <- sprintf("ITT_Analysis/results/figure3_piecewise_2yr_%s.png", ts)
