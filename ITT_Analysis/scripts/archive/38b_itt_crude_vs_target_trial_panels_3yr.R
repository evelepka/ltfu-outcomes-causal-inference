...
p_crude <- ggplot(td_crude, aes(x = time, y = 1 - estimate, color = strata)) +
  ...
  labs(title = "Panel A: Crude 3-Year Mortality", subtitle = "Anchored at Tx Day 0",
...
generate_tt_plot <- function(data, m) {
  ...
  anno_text <- sprintf("Early aHR: %.2f (%.2f-%.2f)\nLate  aHR: %.2f (%.2f-%.2f)", e1_hr, e1_l, e1_u, e2_hr, e2_l, e2_u)
  ...
  p <- ggplot(td, aes(x = time, y = 1 - estimate, color = strata)) +
    ...
    geom_step(linewidth = 1) +
    annotate("text", x = 0.55, y = 0.13, label = anno_text, hjust = 0, vjust = 1, size = 3) +
    theme_classic() +
...
out_name <- sprintf("ITT_Analysis/results/figure2_crude_panels_3yr_%s.png", ts)
