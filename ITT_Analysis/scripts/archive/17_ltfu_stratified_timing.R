# 17_ltfu_stratified_timing.R
# Stratified mortality analysis by Treatment Month of LTFU
...
# Ensure tx_month_grp is a factor with correct order
ltfu_data$tx_month_grp <- factor(ltfu_data$tx_month_grp, 
                                 levels = c("< 1 month", "1 to <2 months", "2 to <4 months", "≥ 4 months"))

# 2. Survival Analysis - Timing of LTFU
fit_time <- survfit(Surv(time_d_tx, event_d) ~ tx_month_grp, data = ltfu_data)

# Log-rank test
sdiff <- survdiff(Surv(time_d_tx, event_d) ~ tx_month_grp, data = ltfu_data)
...
results_table <- get_vals(fit_time, c(1, 5, 10, 11.9))

print("--- Cumulative Mortality Incidence (%) in LTFU Group by Timing ---")
print(results_table)
...
plot_df <- tidy_surv(fit_time)
plot_df$mortality <- 1 - plot_df$surv
plot_df$timing_status <- gsub("tx_month_grp=", "", plot_df$strata)
plot_df$timing_status <- factor(plot_df$timing_status, levels = levels(ltfu_data$tx_month_grp))

p <- ggplot(plot_df, aes(x = time, y = mortality, color = timing_status)) +
  geom_step(linewidth = 1.2) +
  theme_classic(base_size = 14) +
  labs(
    title = "Cumulative incidence of mortality by timing of loss to follow-up",
...
ggsave(paste0(out_dir, "Mortality_KM_12y_by_TimingLTFU.png"), p, width = 8, height = 6, dpi = 300)
write.csv(results_table, paste0(out_dir, "ltfu_mortality_timing_results.csv"), row.names = FALSE)
