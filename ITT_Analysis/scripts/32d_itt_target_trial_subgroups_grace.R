# 32d. Target-trial subgroup HRs (age/sex/HIV/homelessness) under 30-day grace
# ==============================================================================
# Mirrors 32b but applies the 30-day grace landmark (as in 30d) so the
# subgroup-stratified estimates are consistent with the grace-period primary
# late-mortality target trial.
#
# Output: target_trial_subgroup_interactions_grace_mi.csv
#         (drop-in replacement for 32b output for grace-period primary
#          analyses; layout identical so Figure 3C can read either file)
# ==============================================================================
...
M <- length(imp_files)
cat(sprintf("[32d] %d imputed datasets; grace = %d d\n", M, GRACE_DAYS))
...
out_path <- file.path(ITT_RESULTS_DIR, "target_trial_subgroup_interactions_grace_mi.csv")
write.csv(final_df, out_path, row.names = FALSE)
cat(sprintf("\n[32d] Wrote %d rows to %s\n", nrow(final_df), out_path))
