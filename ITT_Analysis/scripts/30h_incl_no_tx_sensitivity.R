...
ITT_MI_DIR <- file.path(ITT_DATA_DIR, "mi_incl_no_tx_start_backup")  # INCLUSIVE sensitivity
...
out_path <- file.path(ITT_RESULTS_DIR, "target_trial_defnB_INCL_array.csv")
...
if (file.exists(ga) && file.exists(or)) {
  ...
  comp_path <- file.path(ITT_RESULTS_DIR, "target_trial_defnB_INCL_vs_defnA.csv")
